...
class Blockchain():
    ...
    def __init__(self,chain=[]):
        ...
        self.chain = chain
    def add(self,block):
        self.chain.append({'hash' : block.hash(),'pervious' : block.previous_hash ,'number' : block.number,'data' : block.data , 'nonce' : block.nonce})
    def mine(self,block):
        try:
        ...
        while True:
            if block.hash() [:4] == "0" * self.diffculity:
...
def main():
   ...
   for data in database:
       num += 1
       blockchain.mine(Block(data,num),Blockchain)
